<template>
    <div class="AddPro">
        <el-dialog title="新增标准" :modal-append-to-body="false" customClass="customClass" :visible.sync="dialogAdd.show">
            <el-form :model="formDate" ref="dialogAdd" label-width="100px" :rules="formrules">
				...
				<div class="flex">
					<el-form-item label="工程类别" class="flex-1 px-2" prop="projectType">
					    <el-select v-model="formDate.projectType" placeholder="请选择工程类别">
					        <el-option v-for="(option,index) in options2" :label="option.label" :value="option.value" :key="index"></el-option>
					    </el-select>
					</el-form-item>
				...
				<div class="flex">
					...
					<!-- <el-form-item label="标准类型" prop="standard" class="flex-1 px-2">
					    <el-input v-model="formDate.standard" placeholder="例如: 国家标准1"></el-input>
					</el-form-item> -->
					<el-form-item label="标准类型" prop="standard" class="flex-1 px-2">
						<el-select v-model="formDate.depotId" placeholder="请选择违章数据库">
...
<script>
	export default {
		...
		props: ['dialogAdd','selectValue','options'],
		// props: {
		// 	dialogAdd: Object,
		// 	options : Object,
		...
		data() {
			return {
				...
				formrules: {
					...
					standard: [{ required: true, message: '标准类型不能为空', trigger: 'blur' }],
				},
				options2: [
					{
					  value: '建筑工程',
		...
		methods: {
			...
			dialogFormAdd() {
				...
				fd.append('standard', this.formDate.standard);
