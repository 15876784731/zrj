			...
			<el-form :model="formDate" ref="dialogEdit" label-width="80px" :rules="formrules">
				...
				<div class="flex">
					<el-form-item label="组长" prop="categories" class="flex-1 px-2">
						...
						<el-input v-model="formDate.categoryIndex"></el-input>
					...
					<el-form-item label="副组长" prop="company" class="flex-1 px-2">
						...
						<el-select v-model="formDate.optionsIndex" placeholder="请选择" style="width: 100%;">
							<el-option v-for="(item,index) in a.groupDeputy" :key="item.value" :label="item.label" :value="index">
							</el-option>
						</el-select>
					</el-form-item>
				</div>
				<div class="flex">
					<el-form-item label="组员" prop="province" class="flex-1 px-2">
						<el-select v-model="formDate.province" placeholder="请选择" style="width: 100%;">
							<el-option v-for="(item,index) in a.groupMember" :key="item.value" :label="item.label" :value="index">
							</el-option>
						</el-select>
					</el-form-item>
				</div>
...
<script>
	export default {
		...
		methods: {
			obtain() {
				this.a.groupLeader = this.tableData[this.number].groupLeader;
				this.a.groupDeputy = this.tableData[this.number].groupDeputy;
				this.a.groupMember = this.tableData[this.number].groupMember;
			},
			//模态框的新增
			dialogFormAdd() {
				const that = this
