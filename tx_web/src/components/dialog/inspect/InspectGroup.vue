<template>
	<div id="ProTable" class="basic container">
		...
		<!-- 编辑弹出框 -->
		<EditInsGroup :dialogEdit="dialogEdit" :form="form" :type="type" :tableData="tableData" :number="number"></EditInsGroup>
		<!-- 新增工程 -->
		<AddInsGroup :dialogAdd="dialogAdd" :form="form" @childFn="parentFn"></AddInsGroup>
	</div>
</template>
<script>
	...
	export default {
		...
		methods: {
						...
						for(let j=0;j<oldArr.length;j++){
							if(newArr[i].groupID == oldArr[j].groupID){
								if(oldArr[j].groupPost == 2){
									...
									console.log(that.tableData[i]['groupDeputy'])
								}else{
			...
			handleDelete(index,row) {
				...
				fd.append('groupName', row.groupName)
				// console.log(this.$loading())
