...
<script>
	export default {
		...
		mounted() {
			...
			this.formDate.groupLeader = this.userInfo.userName
			...
			console.log(this.formDate.groupLeader)
		},
		methods: {
			//获取所有用户
			getUser(){
			...
			dialogFormAdd() {
				...
				console.log( this.groupLeaderValue)
			...
			FormTabelEmpty() {
				for (var i in this.formDate) {
					this.formDate[i] = '';
					// this.formDate[i].groupDeputy = '';
					// this.formDate[i].groupMember = '';
				}
				this.formDate.groupLeader = this.userInfo.userName
