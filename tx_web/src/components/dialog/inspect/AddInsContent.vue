	...
	<div class="AddPro">
		<el-dialog title="新建任务检查项" :modal-append-to-body="false" customClass="customClass" :visible.sync="dialogAdd.show">
			<el-form :model="formData" ref="dialogAdd" label-width="100px" :rules="formrules">
				...
				<div class="flex">
					<el-form-item label="检查负责人" class="flex-1 px-2">
						<el-select multiple collapse-tags v-model="formData.GroupMembers" placeholder="请选择" style="width: 100%;">
							<el-option v-for="item in GroupMembersList" :key="item.key" :label="item.label" :value="item.value">
							</el-option>
						</el-select>
					</el-form-item>
				</div>
...
<script>
	export default {
		...
		mounted() {
			console.log(1111)
			let inspectDefineData1 = sessionStorage.getItem('inspectDefineData')
			this.inspectDefineData1 = JSON.parse(inspectDefineData1)
			console.log(this.inspectDefineData1)
			this.getTableCate()
			this.getInsTask()
		},
		methods: {
			// 获取小组成员
			getInsTask() {
				console.log(this.inspectDefineData1.inspectGroup)
				const that = this
				let fd = new FormData()
				fd.append('flag', 'GroupMembers')
				fd.append('inspectGroup',this.inspectDefineData1.inspectGroup)
				that.$axios.post(that.$adminUrl + `/inspect/InspectAdmin.php`, fd).then(res => {
					console.log(res)
					that.tableData = res.data.data
					console.log(this.tableData)
					for(let i = 0;i < that.tableData.length;i++) {
					that.GroupMembersList.push({
						value: this.tableData[i].name + '/' + this.tableData[i].phnoeNumber+ '/' + this.tableData[i].company,
						label: this.tableData[i].name + '/' + this.tableData[i].phnoeNumber+ '/' + this.tableData[i].company
					})
				}
				console.log(that.GroupMembersList)
				}).catch(() => {
					console.error("获取失败")
				})
