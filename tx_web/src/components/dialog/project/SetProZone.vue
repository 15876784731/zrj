						 ...
						 header-cell-class-name="table-header" :data="table" stripe>
							...
							<el-table-column label="操作" align="center">
								<template slot-scope="scope">
									...
									<!-- <el-button type="text" icon="el-icon-edit" @click="setFloor(scope.$index, scope.row)">已定义</el-button> -->
		...
		methods: {
			...
			dialogFormAdd(){
				...
				for (let i = 0; i < this.tableData.length; i++) {
					console.log(this.tableData[i].unitNum);
					if (this.tableData[i].unitNum == undefined) {
						
						this.$confirm('未进行栋号定义, 确认将无法保存，是否继续?', '提示', {
          					confirmButtonText: '确定',
         					cancelButtonText: '取消',
          					type: 'warning'
        				}).then(() => {
