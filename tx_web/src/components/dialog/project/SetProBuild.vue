						 ...
						 header-cell-class-name="table-header" :data="tableData" stripe>
							...
							<el-table-column prop="unitName" label="实测单名称" align="center">
								<template slot-scope="scope">
									...
									<!-- <el-button  type="text" icon="el-icon-edit" @click="defineUnitName(scope.$index, scope.row)">已定义</el-button> -->
								</template>
							</el-table-column>
	...
	export default {
		...
		data() {
			return {
				formDate:{
					UnitName:'',
					definedUnitName:'true',
					undefinedUnitName:'false',
					inspectDate:'',
					inspectItem:'',
					inspectPerson:'',
                    inspectPosition:'',
		...
		methods: {
			getMsgFormSon(data) {
				...
				// if (this.UnitName == '111') {
				// 	this.definedUnitName = false
				// 	this.undefinedUnitName = true
				// }
				// else {
				// 	this.definedUnitName = true
				// 	this.undefinedUnitName = false
				// }
			},
			handleChange (row) {
			...
			defineUnitName(index,row){
				...
				// sessionStorage.setItem('buildForm',JSON.stringify(buildForm))
			},
			//删除楼栋
