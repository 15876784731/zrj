<template>
	<div class="basic" style="flex:1;display:flex=1" id="projectManage">
			...
			<el-tabs v-model="activeName" type="card" @tab-click="handleClick">
				<el-tab-pane label="建筑工程" name="first">
					<div class="handle-box">
						...
						<template>
							<el-select v-model="selectValue" placeholder="请选择" class="custom-select" @change="getArchitecture">
		...
		<!-- 编辑弹出框 -->
		<EditStandardItem :dialogEdit="dialogEdit"  :selectValue="selectValue" :form="form" @handleUpdateInspectionItemChild="handleAddInspectionItem" :type="type"></EditStandardItem>
		<!-- 批量导入 -->
		<!-- <Import :dialogImport="dialogImport" :form="form" @updateImp="initWebSocket"></Import> -->
		<!-- 新增标准条目 -->
		<AddStandardItem :dialogAdd="dialogAdd" :selectValue="selectValue" :options="options" @handleAddInspectionItemChild="handleAddInspectionItem"></AddStandardItem>
		<!-- 赋予权限 -->
		...
		methods: {
			...
			handleAddDepot() {
				this.$prompt('请输入自定义库名', '新增自定义库', {
					...
					const that = this
					let fd = new FormData()
					fd.append('flag','addDepot2')
					fd.append('standardName',value)
			...
			getDepot() {
				...
				that.$axios.post(that.$adminUrl + `/system/Database.php`, fd).then(res => {
					...
					for(let i =0;i<res.data.data.length;i++){
						that.options.push({
							value:res.data.data[i].standardName,
							label:res.data.data[i].standardName
						})
					}
				}).catch({
			...
			commentDelete(row_id) {
				...
				this.$confirm('此操作将永久删除该信息, 是否继续?', '提示', {
					...
					fd.append('tableType', 'tb_system_standard')
					fd.append('id', row_id)
					// fd.append('account',localStorage.getItem('ms_username'))
			...
			getArchitecture() {
				...
				fd.append('tableType', 'tb_system_standard')
				fd.append('selectValue', that.selectValue)
