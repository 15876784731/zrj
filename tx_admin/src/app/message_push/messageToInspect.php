	...
	switch($flag){
		...
		case 'InspectMessagePush':
            ...
            if($result1->num_rows>0)	{
				$sql3="UPDATE tb_inspectaccept_notice SET state = '待整改' WHERE noticeTimeStamp='$cardTimeStamp' ";
				$result3 = $conn->query($sql3);
        	 	foreach($Person as $receiver){
        	 		$receiver = explode('|', $receiver);
        	 		$receiverPhone = $receiver[1];
        	 		$receiverid = $receiver[3];
        	 		$sql2="INSERT INTO tb_msg_notice SET timeStamp = '$timeStamp', title = '项目检查回复' , `content` = '$content',cardTimeStamp = '$cardTimeStamp', `time` = '$time', `read` = '0', `delete` = '0', `receiver` = '$receiverPhone',`userId` = '$receiverid', `projectId` = '$projectId' , `messagetype` = '项目检查', `position` = '$inspectPosition', `positiontype` = '整改' ,`comefrom` = '$comefrom'";
