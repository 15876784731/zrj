<?php
	...
	if($flag=="getArchitectureData"){
		...
		$sql = "SELECT * FROM ".$tableType." WHERE `projectType`='建筑工程' AND standard ='$selectValue'";
		...
		$tableType = isset($_POST["tableType"])?$_POST["tableType"]:'';
		$sql = "SELECT * FROM ".$tableType." WHERE `projectType`='装修工程' AND standard ='$selectValue'";
		$result = $conn->query($sql);
		if($result->num_rows>0)	{
			$res = array();
			$resData = array();
		...
		$tableType = isset($_POST["tableType"])?$_POST["tableType"]:'';
		$sql1 = "SELECT * FROM ".$tableType." WHERE projectType='$projectType' AND inspectionItem='$inspectionItem' AND inspectionContent='$inspectionContent' AND number='$number' AND miniStandardValue='$miniStandardValue' AND maxStandardValue='$maxStandardValue' AND measurementType='$measurementType' AND standard='$standard'";
		$result1 = $conn->query($sql1);
		if($result1->num_rows==0){
			$sql = "INSERT INTO ".$tableType." SET projectType = '$projectType',inspectionItem = '$inspectionItem',inspectionContent = '$inspectionContent',number = '$number',miniStandardValue = '$miniStandardValue',maxStandardValue = '$maxStandardValue',measurementType = '$measurementType' AND standard='$standard'";
		}
		$result = $conn->query($sql);
		if(!$result){
			$data['code'] = 0;
		...
		$standard = isset($_POST["standard"])?$_POST["standard"]:'';
		$tableType = isset($_POST["tableType"])?$_POST["tableType"]:'';
		$sql = "UPDATE ".$tableType." set  projectType = '$projectType',inspectionItem = '$inspectionItem',inspectionContent = '$inspectionContent',number = '$number',miniStandardValue = '$miniStandardValue',maxStandardValue = '$maxStandardValue',measurementType = '$measurementType',standard = '$standard' WHERE id='$id'";
		$result = $conn->query($sql);
		if(!$result){
			$data['code'] = 0;
		}
