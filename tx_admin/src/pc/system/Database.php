<?php
	...
	switch($flag){
		...
		//新增标准库
		case 'addDepot2':
			$standardName = isset($_POST["standardName"]) ? $_POST["standardName"] : '';
			
			$sql = "SELECT * from tb_system_standard_depot where standardName = '$standardName'";
			$res = $conn -> query($sql);
			if ($res -> num_rows > 0) {
				$ret_data["states"] = 'error';
			}else{
				$sql = "INSERT INTO tb_system_standard_depot set `standardName`='$standardName' ";
				$res = $conn -> query($sql);
				$ret_data["states"] = 'success';
			}
			break;
		//获取标准库
		case 'getDepot2':
			$sql = "SELECT * from tb_system_standard_depot order by id asc";
			$res = $conn -> query($sql);
			if ($res -> num_rows > 0) {
				$i = 0;
				while ($row = $res -> fetch_assoc()) {
					$ret_data["data"][$i]["id"] = $row["id"];
					$ret_data["data"][$i]["standardName"] = $row["standardName"];
