<?php
	...
	$inspectGroup = isset($_POST["inspectGroup"])?$_POST["inspectGroup"]:'';
	$ret_data = array();
	switch($flag){
		//获取巡查小组信息
		case 'getInsTask':
		...
		case 'GroupMembers':
			...
			$sql="SELECT * FROM `tb_quality_group_member` WHERE groupName = '$inspectGroup'"; 
			$result = $conn->query($sql);
			if ($result->num_rows > 0) {
				$i = 0;
				while ($row = $result -> fetch_assoc()) {
					$ret_data["data"][$i]["name"] = $row["name"];
					$ret_data["data"][$i]["phnoeNumber"] = $row["phnoeNumber"];
