	...
	switch($flag) {
		...
		case 'saveInsItems':
			...
			foreach($inspectQues  as $obj){
				...
				// $ret_data["states"] = 'success';
				if($res -> num_rows > 0){
					$sql = "INSERT INTO tb_quality_inspect_items set `taskId` = '$taskId',`inspectCate` = '$inspectCate',`inspectQues` = '$question',`deductTableCate` = '$deductTableCate',`GroupMembers` = '$GroupMembers',fullMarks = '$fullMarks' ";
					$result = $conn->query($sql);
					$ret_data["states"] = 'success';
				}else{
					$sql = "INSERT INTO tb_quality_inspect_items set `taskId` = '$taskId',`inspectCate` = '$inspectCate',`inspectQues` = '$question',`deductTableCate` = '$deductTableCate',`GroupMembers` = '$GroupMembers',fullMarks = '$fullMarks' ";
