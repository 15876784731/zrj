...
//					$ret_data["data"][$i]["zone"] = $row["zone"];
		...
		//删除
		case 'delete':
			$sql = "DELETE FROM tb_project WHERE `timeStamp`= '$timeStamp'";
		...
		case 'addProject':
			...
			$res = $conn -> query($sql);
			if ($res -> num_rows ==0) {
				$sql = "INSERT INTO tb_project (timeStamp,projectName,categories,company,companyId,province,city,proPosition,architecture,startTime,completedTime,proState) VALUES ('$timeStamp','$projectName','$categories','$company','$companyId','$province','$city','$proPosition','$architecture','$startTime','$completedTime','未竣工') ";
				$res = $conn->query($sql);
		...
		case 'editProject':
			...
			$res = $conn -> query($sql);
			if ($res -> num_rows > 0) {
				$sql = "UPDATE tb_project SET isCompleted = '1',proState = '已竣工' where timeStamp='".$timeStamp."'";
				$res = $conn->query($sql);
				$ret_data["states"] = 'success';
				$ret_data["sql"] = $sql;
			}else{
