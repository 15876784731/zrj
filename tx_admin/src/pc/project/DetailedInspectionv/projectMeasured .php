<?php
	...
	switch($flag){
		// 实测实量表格查询
		case 'MeasuredQuantity1':
			$sql = "SELECT * FROM tb_inspectaccept_measure where `measureState`= '未完成' order by id desc";
			$res = $conn -> query($sql);
			if ($res->num_rows > 0) {
				$i = 0;
...
//					$ret_data["data"][$i]["city"] = $row["city"];
			...
			case 'MeasuredQuantity2':
				$sql = "SELECT * FROM tb_inspectaccept_measure where `measureState`= '已完成' order by id desc";
				$res = $conn -> query($sql);
				if ($res->num_rows > 0) {
					$i = 0;
					while ($row = $res -> fetch_assoc()) {
