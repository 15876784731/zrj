<template>
	<view style="padding: 20rpx;">
		<!-- <filter-popup  v-model="showPopup" title="全部筛选" :projectInfo="projectInfo"  height="950rpx" @finsh="subFinsh" @reset="subReset"></filter-popup> -->
		<uni-collapse :accordion="true">
			<uni-collapse-item v-for="item in accordion" :open="item.animation" :key="item.id" :title="item.title" :show-animation="true">
...
<script>
	...
	export default {
		...
		onLoad:function(option){
			...
			const procedureInfo = JSON.parse(option.procedureInfo);
			// console.log(this.photo);
			console.log(procedureInfo);
			this.formData.category = procedureInfo.category;
			this.formData.inspectItem = procedureInfo.inspectItem;
			this.formData.inspectProcedure = procedureInfo.inspectProcedure;
			this.formData.inspectContent = procedureInfo.inspectContent;
		...
		methods: {
			...
			getStoragedata() {
				...
				uni.getStorage({
					...
					success: function (res) {
						...
						that.getViolationItemFunc()
					}
				});
				uni.getStorage({ // 获取检查部位
					key: 'buildInfo',
					success: function (res) {
						const buildInfo = res.data
						// console.log(res.data)
						that.formData.inspectPosition = buildInfo.section + '-' +buildInfo.build + '-' + buildInfo.floor + '-' + buildInfo.unit;
					}
				});
				// 获取用户信息
				uni.getStorage({
					...
					success:function(res) {
						...
						that.formData.endDate = '';
						that.formData.inspectDate = '';
			...
			setthisdata(){
					...
					let rndVal = this.commonFunc.RndNum(2)
			...
			getViolationItemFunc(option) {
				// if (option!=undefined) {
				// 	this.formData.inspectProcedure = option.inspectProcedure;
				// 	this.formData.inspectContent = option.inspectContent;
				// 	this.formData.inspectItem = option.inspectItem;
				// } else {
			...
			dialogInputConfirm(done, val) {
				...
				this.myRequest.httpRequest(opts,param,isLoading).then(res=>{
					console.log(res.data)
					this.accordion[0].animation = true;
					if(res.data.data.commonlyQuestion!=undefined){
						let itemsArr = res.data.data.commonlyQuestion
						for(let i=0;i<itemsArr.length;i++){
			...
			trigger(e) {
				...
				if(indexOf==0){//查询事件
				...
				}else if (indexOf == 1){//提交事件
					// console.log(this.checkedList0);
					// console.log(this.checkedList1);
					// console.log(this.checkedList2);
					const checkedList = [...this.checkedList0,...this.checkedList1,...this.checkedList2]
					this.formData.violationItem = checkedList
					console.log(this.formData);
					...
					let param = {
						...
						formData: data,
						projectId: projectId,
						number: this.formData.inspectContent
					}
					let isLoading = false
					this.myRequest.httpRequest(opts,param,isLoading).then(res=>{
						console.log(res);
			...
			// subFinsh (obj) {
			// 	console.log(obj);
			// 	this.getViolationItemFunc(obj)
				
			// },
			// subReset (obj) {
			// 	this.getViolationItemFunc()
			// },
