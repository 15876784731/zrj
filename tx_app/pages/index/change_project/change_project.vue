			    ...
			    <view class="uni-list-cell">
			        ...
			        <view class="uni-list-cell-db">
			            <picker @change="pickCompany" :value="companyIndex" :range="companyList">
							<input :class="{'uni-input':true, 'border-col-blue': !companyName}" type="text" :value="companyName" placeholder="请选择公司" disabled="false"/>
			            </picker>
			        </view>
			    </view>
			</view>
		    ...
		    <view class="uni-list-cell">
		        ...
		        <view class="uni-list-cell-db">
		            <picker @change="pickProject" :value="projectIndex" :range="projectNameList">
						<input :class="{'uni-input': true, 'border-col-blue': !projectName}" type="text" :value="projectName" placeholder="请选择工程项目" disabled="false"/>
		            </picker>
		        </view>
		    </view>
		</view>
					...
					<view class="list-item" @tap="handleTap('picker2')">
						<view class="item-content ">
							<view class="item-value ">
								...
								<input :class="{'uni-input':true, 'border-col-blue': !label2}" type="text" :value="label2" placeholder="请选择区段栋号" disabled="false"/>
							</view>
						</view>
			...
			<view class="uni-list-cell">
				...
				<view class="uni-list-cell-db">
					<view class="list-item" @tap="handleTap('picker3')">
						<view class="item-content">
							<view class="item-value">
								<input :class="{'uni-input':true, 'border-col-blue': !label3}"  type="text" :value="label3" placeholder="请选择楼层单元" disabled="false"/>
...
<script>
	...
	export default {
		...
		onLoad: function(option){
			...
			this.projectName = this.userInfo.projectName
			this.proTimeStamp = this.userInfo.proTimeStamp
			if (this.proTimeStamp) {
				const buildInfo = JSON.parse(option.buildInfo);
				this.getBuildInfo();
				if (buildInfo) {
					this.label2 = buildInfo.section + '-' + buildInfo.build;
					this.label3 = buildInfo.floor + '-' + buildInfo.unit;
					this.getFloorUnit();
				}
			}
			// uni.getStorage({
	    ...
	    methods: {
						...
						if(this.projectNameList!=''){//选中缓存中的
							if(length>=1){
								for(var i =0;i<length;i++){
									...
									if(data[i].projectName==this.projectNameList[0]){
										
									}else{
										this.projectNameList.push(data[i].projectName)
									}
									if(data[i].projectId==this.projectList[0].projectId){
			...
			pickProject: function(e){
				...
				console.log(this.projectList[this.projectIndex]);
