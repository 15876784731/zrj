	...
	export default {
		...
		methods: {
			...
			clicknotice(item) {
				...
				let positiontype = item.positiontype;
				let comefrom = item.comefrom;
				if (item.read === '0') {
					//改变已读
					let opts = {
						url: this.api + '/msg/TodoNotice.php',
				...
				switch (messagetype) {
					...
					case '项目检查':
						uni.navigateTo({
							url: `../module_project/InspectAccept/newInspect/CallBack/RectificationReply?cardTimeStamp=${cardTimeStamp}`+`&positiontype=${positiontype}`+`&comefrom=${comefrom}`+`&msgid=${msgid}`
						})
						break;
					case '项目验收':
						uni.navigateTo({
